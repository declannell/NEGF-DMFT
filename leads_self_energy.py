...
class SelfEnergy: 
    ...
    def get_transfer_matrix(self,k_y): #this assume t and t_tilde are the same and principal_layer is 1 atom thick
        t_0 = [0 for r in range( 0 , parameters.steps() )]
        t_next = [0 for r in range( 0 , parameters.steps() )]
        a = [0 for r in range( 0 , parameters.steps() )]
        
        if( parameters.chain_length_y() == 1 ):
            for r in range( 0 , parameters.steps() ):                
                t_next[r] = parameters.hopping_lx() / (parameters.energy()[r] - parameters.onsite_l() )
                a[r] = t_next[r]
            ...
            for r in range( 0 , parameters.steps() ):
                t_0[r] = parameters.hopping_lx() /( parameters.energy()[r] - parameters.onsite_l() -2 * parameters.hopping_ly() * np.cos(k_y))
                a[r] = t_0[r]
                t_next[r] = t_0[r]
    ...
    def sgf( self , num , k_y ):
        if( parameters.chain_length_y() == 1 ):
            for r in range( 0 , parameters.steps() ):
                self.surface_gf_l[r][num][0][0] = 1 / (parameters.energy()[r] - parameters.onsite_l() - parameters.hopping_lx() * self.transfer_matrix[r][0][0] )
       
        else:
            for r in range( 0 , parameters.steps ):
                self.surface_gf_l[r][num][0][0] = 1 / (parameters.energy()[r] - parameters.onsite_l() - 2 * parameters.hopping_ly() * np.cos(k_y) - parameters.hopping_lx() * self.transfer_matrix[r][0][0] )

    def lead_self_energy(self, num):
        for r in range(0,  parameters.steps() ):
            self.self_energy_left[r][num][0][0] = parameters.hopping_lc()**2 * self.surface_gf_l[r][num][0][0]
            self.self_energy_right[r][num][0][0] = parameters.hopping_lc()**2 * self.surface_gf_l[r][num][0][0]
...
def analytic_se():
    ...
    for i in range(0, parameters.steps() ):
        x=( parameters.energy()[i].real - parameters.onsite_l() ) / ( 2 * parameters.hopping_lx() )
        #print(x, energy[i])
