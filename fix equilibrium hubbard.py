...
def gf_initial_calculator( hamiltonian , parameters, energy):
...
def gf_calculator(  hamiltonian  , self_energy, parameters, energy):

    inverse_gf=create_matrix(parameters.chain_length)
    for i in range(0,parameters.chain_length):
           for j in range(0,parameters.chain_length): 
               if(i==j):                
                   inverse_gf[i][j]=-hamiltonian.efffective_matrix[i][j]+energy+self_energy[i][j]
               else:
                   inverse_gf[i][j]=-hamiltonian.efffective_matrix[i][j]
    return la.inv(inverse_gf, overwrite_a=False, check_finite=True)  
...
def get_self_consistent_occup(parameters,  energy ):
    ...
    for r in range(0, parameters.steps):
        gf_initial_up[r]=gf_initial_calculator(hamiltonian_up, parameters, energy[r])
        gf_initial_down[r]=gf_initial_calculator(hamiltonian_down, parameters, energy[r])      
