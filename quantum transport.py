...
class HubbardHamiltonian: #this is the hamiltonian of the scattering region
    ...
    def __init__(self):
        ...
        self.efffective_hamiltonian = [create_matrix(parameters.chain_length) for r in range(parameters.steps)]
        ...
        for r in range(0 , parameters.steps):
            ...
            for i in range(0 , parameters.chain_length):
                ...
                for j in range(0 , parameters.chain_length):  
                    self.efffective_hamiltonian[r][i][j] = self.matrix[i][j] + self.self_energy_right[r][i][j] + self.self_energy_left[r][i][j]

    def embedding_self_energy(self ): #this function should work as the analytic case is correct
   ...
   # plt.plot(energy,analytic, color='tomato')
    ...
    def print(self, num):
        for i in range(0,parameters.chain_length):
            row_string = " ".join((str(r).rjust(5, " ") for r in self.efffective_hamiltonian[num][i])) #rjust adds padding, join connects them all
            print(row_string)
...
def integrate(gf_1, gf_2, gf_3, r):# in this function, the green functions are 1d arrays in energy. this is becasue we have passed the diagonal component of the green fun( lesser, or retarded) 
    ...
    for i in range(0 , parameters.steps):
        for j in range(0 , parameters.steps): #this integrates like PHYSICAL REVIEW B 74, 155125 2006
            # I say the green function is zero outside -14 and +14. This means I need the final green function in the integral to be within -14 and 14. The index of 0 corresponds to -14.
...
def green_lesser_fluctuation_dissapation(green_function): #this is only used to compare the lesser green functions using two different methods
...
def self_energy_calculator( gf_up, gf_down, g_lesser_up , g_lesser_down ):# this creates the entire energy array at once
    ...
    for r in range(0 , parameters.steps):# the are calculating the self energy sigma_{ii}(E) for each discretized energy. To do this we pass the green_fun_{ii} for all energies as we need to integrate over all energies in the integrate function
        for i in range(0 , 1):
            self_energy[r][i][i] =  parameters.hubbard_interaction ** 2 * ( integrate( [ e[i][i] for e in gf_up] , [ e[i][i] for e in gf_down]  , [ e[i][i] for e in g_lesser_down]   , r )  )
            self_energy[r][i][i] += parameters.hubbard_interaction ** 2 * ( integrate( [ e[i][i] for e in gf_up] , [ e[i][i] for e in g_lesser_down] , [ e[i][i] for e in g_lesser_down]  ,r  ) ) 
            self_energy[r][i][i] += parameters.hubbard_interaction ** 2 * ( integrate( [ e[i][i] for e in g_lesser_up] , [ e[i][i] for e in gf_down] , [ e[i][i] for e in g_lesser_down]  ,r  ) ) 
            self_energy[r][i][i] += parameters.hubbard_interaction ** 2 * ( integrate( [ e[i][i] for e in g_lesser_up] , [ e[i][i] for e in g_lesser_down]  , [conjugate( e[i][i]) for e in gf_down]  ,r  ) ) #fix advanced green function
    return self_energy

def lesser_embedding(): #this function should work as it the same for non interacting case as the analytic solution
...
def green_function_calculator( hamiltonian , self_energy ,  energy, energy_step):#this calculates the green function for a specific energy point.
    inverse_green_function=create_matrix(parameters.chain_length)
    for i in range( 0 , parameters.chain_length ):
           for j in range( 0 , parameters.chain_length ): 
               if( i == j ):                
                   inverse_green_function[i][j] = - hamiltonian.efffective_hamiltonian[energy_step][i][j] - self_energy[i][j] + energy
               else:
                   inverse_green_function[i][j] = - hamiltonian.efffective_hamiltonian[energy_step][i][j]
...
def get_spin_occupation( gf_lesser_up , gf_lesser_down ): #this should work as in second order it gives the same result as fluctuation dissaption thm to 11 deciaml places
    delta_energy = (parameters.e_upper_bound - parameters.e_lower_bound )/parameters.steps
...
"""   
def lesser_se_mb_eq( gf_r_up , gf_r_down ): #this isnt used anywhere in the code
    self_energy_up_lesser = [ create_matrix(parameters.chain_length) for i in range( parameters.steps ) ]   
...
def compare_g_lesser( g_lesser_up, gf_int_up):
    lesser_g = green_lesser_fluctuation_dissapation( gf_int_up )
...
def embedding_self_energy_retarded():
